"""
Rug Analyzer - Identifies patterns for potential -20% stock drops
Reuses existing stock analyzer logic with rug-specific scoring weights
"""
...
from ..services.model_loader import predict_rug_ml, get_model_loader
...
@dataclass
class RugAlert:
    """Alert for potential rug (>20% drop) warning"""
    symbol: str
    company_name: str
    confidence: float
    reasons: List[str]
...
class RugAnalyzer:
    """
    Analyzer for identifying "When Rug?" patterns.
    ...
    async def analyze_rug_potential(self, symbol: str, company_name: str = None) -> Optional[RugAlert]:
        """
        ...
        Returns:
            RugAlert if confidence > threshold, None otherwise
        """
        try:
    ...
    async def _predict_with_ml_model(self, technical_data: Dict, news_data: Dict, social_data: Dict) -> Optional[Tuple[float, Dict, Dict]]:
        ...
        try:
            ...
            # Get ML prediction
            ml_confidence, ml_details = await predict_rug_ml(features)
    ...
    async def _create_ml_alert(self, symbol: str, company_name: str, ml_confidence: float,
                              ml_details: Dict, features: Dict, technical_data: Dict, news_data: Dict) -> RugAlert:
    ...
    async def _analyze_with_rules(self, symbol: str, company_name: str, technical_data: Dict,
                                 news_data: Dict, social_data: Dict) -> Optional[RugAlert]:
...
async def analyze_rug_potential(symbol: str, company_name: str = None) -> Optional[RugAlert]:
    """Convenience function to analyze rug potential"""
    async with RugAnalyzer() as analyzer:
        return await analyzer.analyze_rug_potential(symbol, company_name)
