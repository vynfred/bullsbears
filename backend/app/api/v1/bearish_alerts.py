...
class RugAnalysisRequest(BaseModel):
    """Request model for rug analysis"""
    symbol: str
    company_name: Optional[str] = None


class RugAnalysisResponse(BaseModel):
    """Response model for rug analysis"""
    symbol: str
    company_name: Optional[str]
    has_alert: bool
...
async def get_rug_alerts(
    ...
    """
    Get rug alerts with optional filtering.
    Returns alerts for potential -20% stock drops.
    """
    try:
        # Build query
        query = db.query(AnalysisResult).filter(
            AnalysisResult.alert_type == AlertType.RUG
        )
...
async def get_latest_rug_alerts(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving latest rug alerts: {str(e)}")


@router.post("/analyze", response_model=RugAnalysisResponse)
async def analyze_symbol_for_rug(
    request: RugAnalysisRequest,
    db: Session = Depends(get_db)
):
    """
    Analyze a specific symbol for rug potential.
    ...
    try:
        ...
        # Run rug analysis
        alert = await analyze_rug_potential(symbol, request.company_name)
        
        if alert:
            return RugAnalysisResponse(
                symbol=symbol,
                ...
                message=f"Rug potential detected with {alert.confidence:.1f}% confidence"
            )
        else:
            return RugAnalysisResponse(
                symbol=symbol,
                ...
                earnings_score=None,
                risk_factors=None,
                message="No significant rug potential detected at this time"
            )
...
@router.get("/stats")
async def get_rug_alert_stats(
    days_back: int = Query(30, ge=1, le=365, description="Days back for statistics"),
    db: Session = Depends(get_db)
):
    """
    Get statistics about rug alert performance.
    """
    try:
        cutoff_date = datetime.now() - timedelta(days=days_back)
...
async def get_rug_alert_by_id(
    ...
    """
    Get a specific rug alert by ID.
    """
    try:
        ...
        if not alert:
            raise HTTPException(status_code=404, detail="Rug alert not found")
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving rug alert: {str(e)}")
