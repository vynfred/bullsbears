// src/components/private/LiveLinesChart.tsx
'use client';
...
export function generateSamplePerformanceData(): PerformanceDataPoint[] {
  ...
  for (let day = 0; day <= 30; day++) {
    ...
    const watchlistBoost = Math.random() * 6 - 1;
    const gutBoost = Math.random() * 8 - 2;
    const communityDrag = Math.random() * 4 - 2;

    data.push({
      ts: Date.now() - (30 - day) * 24 * 60 * 60 * 1000,
      price: 100 + baseReturn,
      locked: day === 0,
      daysSincePick: day,
...
export default function LiveLinesChart({
  data,
  timeRange,
  activeFilter,
  onFilterChange,
  ...
  const formatXAxisLabel = (tickItem: any) => `${tickItem}d`;
  const formatYAxisLabel = (value: number) => `${value > 0 ? '+' : ''}${value.toFixed(0)}%`;

  return (
    <div className={`w-full ${className}`}>
      <ResponsiveContainer width="100%" height={320}>
        <LineChart data={filteredData} margin={{ top: 10, right: 15, left: 0, bottom: 10 }}>
          <CartesianGrid strokeDasharray="3 3" stroke="#374151" opacity={0.3} />
          <XAxis dataKey="daysSincePick" tickFormatter={formatXAxisLabel} stroke="#9CA3AF" fontSize={12} axisLine={false} tickLine={false} />
          <YAxis tickFormatter={formatYAxisLabel} stroke="#9CA3AF" fontSize={12} axisLine={false} tickLine={false} width={35} />
          <ReferenceLine y={0} stroke="#6B7280" strokeDasharray="2 2" opacity={0.5} />
          <Tooltip content={<MiniCard />} cursor={{ stroke: '#6B7280', strokeWidth: 1, strokeDasharray: '3 3' }} />
      ...
      <div className="mt-4 flex items-center justify-center gap-4 text-sm overflow-x-auto">
        <div className="flex items-center gap-4 min-w-max">
          {[
            { id: 'all', label: 'ALL PICKS', count: totalPicks, lineColor: '#9CA3AF', textColor: 'text-gray-400' },
            { id: 'gut-only', label: 'YOUR GUT', count: Math.floor(totalPicks * 0.4), lineColor: '#F97316', textColor: 'text-orange-400' },
            { id: 'ai-only', label: 'AI-ONLY', count: Math.floor(totalPicks * 0.8), lineColor: '#3B82F6', textColor: 'text-blue-400' },
            { id: 'watchlist', label: 'WATCHLIST', count: Math.floor(totalPicks * 0.3), lineColor: '#EAB308', textColor: 'text-yellow-400' }
          ].map((filter) => (
            <button
              key={filter.id}
            ...
            >
              <div className="w-3 h-3 rounded-full" style={{ backgroundColor: filter.lineColor }}></div>
