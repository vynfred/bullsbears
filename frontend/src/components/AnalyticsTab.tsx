...
import { demoHistoryEntries, demoAccuracyTrend } from "../lib/demoData";
...
// Generate accuracy data from demo accuracy trend data
const generateAccuracyData = (timePeriod: string = "30d") => {
  ...
  // Filter and transform the demo accuracy trend data
  const filteredData = demoAccuracyTrend
    .filter(point => point.date >= startDate)
    .map(point => ({
      date: point.date.toLocaleDateString('en-US', { month: 'numeric', day: 'numeric' }),
      accuracy: point.combinedAccuracy, // Use combined accuracy for the main line
      picks: point.totalVotes,
      moonHits: Math.floor(point.combinedAccuracy * 0.15) + Math.floor(Math.random() * 5) + 3 // Realistic moon hits based on accuracy
    }));

  // If we don't have enough data points, pad with generated data
  if (filteredData.length < 5) {
    const baseAccuracy = 68.2;
    const targetAccuracy = 72.4;
    const dataPoints = timePeriod === "7d" ? 7 : timePeriod === "30d" ? 15 : 18;
    const accuracyGrowth = (targetAccuracy - baseAccuracy) / (dataPoints - 1);

    return Array.from({ length: dataPoints }, (_, i) => {
      const date = new Date(startDate);
      date.setDate(startDate.getDate() + Math.floor((i * (now.getTime() - startDate.getTime())) / (dataPoints - 1) / (24 * 60 * 60 * 1000)));

      const progressAccuracy = baseAccuracy + (accuracyGrowth * i);
      const variance = Math.sin(i * 0.3) * 1.2;
      const finalAccuracy = Math.min(75, Math.max(65, progressAccuracy + variance));
...
export function AnalyticsTab({ onNavigateToPicks }: AnalyticsTabProps) {
  ...
  const [accuracyOverTimeData, setAccuracyOverTimeData] = useState(() => generateAccuracyData("90d"));

  // Update chart data when time period changes
  useEffect(() => {
    setAccuracyOverTimeData(generateAccuracyData(timePeriod));
  }, [timePeriod]);
    ...
    <div className="space-y-6">
          ...
          >
            <div className="w-full h-full rounded-full flex flex-col items-center justify-center relative" style={{ backgroundColor: 'var(--bg-primary)' }}>
              ...
              <div className="relative z-10 flex flex-col items-center justify-center h-full">
                ...
                <div className="text-xs text-blue-400 mt-2">(145 total picks)</div>
              </div>
            </div>
          </motion.div>
        </div>
      ...
      <Card className="shadow-xl bg-gradient-to-br from-purple-950/60 via-slate-900/40 to-pink-950/30 border-2 border-purple-700/40">
        ...
        <CardContent className="px-0 pb-4">
          <ChartContainer config={chartConfig} className="h-[300px] w-full relative">
              ...
              <LineChart data={accuracyOverTimeData} margin={{ top: 5, right: 15, left: 0, bottom: 5 }}>
                ...
                <ChartTooltip
                  ...
                  labelFormatter={(label) => {
                    const dataPoint = accuracyOverTimeData.find(d => d.date === label);
                    const moonHits = dataPoint?.moonHits || 0;
                    const picks = dataPoint?.picks || 0;
                    return `${label}: ${moonHits} moon hits, ${picks} picks`;
                  }}
                />
                <Line
                  type="monotone"
                  ...
                  dot={(props) => {
                    ...
                    const isLastPoint = index === accuracyOverTimeData.length - 1;
