...
import { ChevronDown, ChevronUp, TrendingUp, TrendingDown, Plus, Clock, Sparkles, ArrowUpDown, Skull, X } from "lucide-react";
...
interface StockPick {
  ...
  timeToTargetHours?: number; // Time in hours from alert to target hit
...
export function PicksTab() {
  ...
  // Combine and filter picks based on time period
  ...
  return (
    <div className="space-y-4">
        ...
        >
          <div className="w-48 h-48 rounded-full bg-gradient-to-br from-emerald-500 via-purple-500 to-rose-500 p-1 shadow-2xl">
            <div className="w-full h-full rounded-full bg-slate-900 flex flex-col items-center justify-center">
              ...
              <div className="text-xs text-slate-400 uppercase">Picks Today</div>
      ...
      {/* Update Status */}
      <Card className="shadow-lg bg-gradient-to-br from-slate-900/60 via-slate-800/40 to-slate-900/30 border-2 border-slate-700/50">
        <CardContent className="py-4">
          <div className="space-y-2 text-center">
            <div className="flex items-center justify-center gap-2 text-sm text-slate-400">
              <Clock className="w-4 h-4" />
              <span>Last updated: 2h 14m ago</span>
            </div>
      ...
      <div className="flex items-center justify-between gap-4">
        <Select value={timePeriod} onValueChange={(value) => setTimePeriod(value as TimePeriod)}>
          <SelectTrigger className="w-[200px] bg-slate-800 border-slate-600 text-slate-100 font-semibold">
            <SelectValue />
          </SelectTrigger>
        ...
        <div className="flex items-center gap-2">
          ...
          <Select value={sortBy} onValueChange={(value) => setSortBy(value as SortOption)}>
            <SelectTrigger className="w-[180px] bg-slate-800 border-slate-600 text-slate-200 font-semibold">
